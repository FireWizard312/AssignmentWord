...
def hangman(secret_word):
    ...
    word_placeholder = []
    for x in secret_word:
        word_placeholder.append(x)
    print("Welcome to the game Hangman!")
    print("I am thinking of a word that is",len(secret_word),"letters long.")
    while amount_of_guesses > 0:
        print("_____________")
        print("You have",amount_of_guesses,"guesses left")
        ...
        if not str.isalpha(guessed_letter):
            warnings -= 1
            if warnings == 0:
                print("Oops! That is not a valid letter. You have no warnings left, so you lost a guess.")
                amount_of_guesses -= 1
            else:
            ...
            warnings -= 1
            if warnings == 0:
                print("Oops! You already guessed this letter. You have no warnings left, so you lost a guess.")
                amount_of_guesses -= 1
            else:
                print("Oops! You already guessed this letter. You have",warnings,"warnings left.")
        else:
            letters_guessed.append(guessed_letter)
            if guessed_letter in word_placeholder:
                if is_word_guessed(secret_word,letters_guessed):
                    return print("Congratulations! You won!")
                else:
                    print("Good guess!")
            else:
                print("Oops! That is not in the my word")
                amount_of_guesses -= 1
    ...
    for i in range(0, len(other_word)):
        if str.isalpha(my_word[i]):
            if other_word[i] != my_word[i]:
                return False
        if other_word.count(other_word[i]) != my_word.count(my_word[i]):
            return False
    return True
...
def show_possible_matches(my_word):
    '''
        ...
        '''
    # FILL IN YOUR CODE HERE AND DELETE "pass"
    #test_word_list = ["ample", "afple", "affle"]
    possible_matches = []
    for x in wordlist:
